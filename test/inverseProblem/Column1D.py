...
ElutionTime  = PFLO.Var(tag="<elutionTime>",\
                        value=L/(FlowVelocity.get_value()/Porosity.get_value())*InjectTimeInPoreVol*3600,\
                        units="s")
...
#RSMD = np.sqrt(np.average(np.power(np.sum(normC,ObservationsRelC*-1),2.)))
RSMD = 1.0

# Plot the final breakthrough curve
plt.figure(figsize=(8,5),facecolor="white")
ax1 = plt.subplot(1,1,1)
ax1.plot(PV,normC,lw=2,c="black")
...
ax1.scatter(whereInPV,ObservationsRelC,marker="o")
